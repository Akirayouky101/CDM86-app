-- ============================================================================
-- CLEANUP: Rimuovi tutti i dati di test precedenti
-- ============================================================================

DO $$
BEGIN
  RAISE NOTICE '🗑️  Pulizia dati test precedenti...';
  
  -- 1. Cancella da referral_network (prima per FK)
  ...
  -- 2. Cancella da points_transactions
  DELETE FROM points_transactions 
  WHERE user_id IN (
    SELECT id FROM users WHERE email LIKE '%mlmtest%'
  );
  
  -- 3. Cancella da user_points
  DELETE FROM user_points 
  WHERE user_id IN (
    SELECT id FROM users WHERE email LIKE '%mlmtest%'
  );
  
  -- 4. Cancella da public.users
  DELETE FROM users WHERE email LIKE '%mlmtest%';
  
  -- 5. Cancella da auth.users
...
-- ============================================================================
-- STEP 1: CREA DIEGO ADMIN (se non esiste)
-- ============================================================================
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🔧 Verifica Diego Admin...';
  ...
  IF v_diego_exists THEN
    ...
    RAISE NOTICE '✅ Diego già esistente, ID: %', v_diego_auth_id;
