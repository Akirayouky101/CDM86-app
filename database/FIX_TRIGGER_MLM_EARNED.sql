-- ============================================================================
-- FIX TRIGGER MLM - Prova valori transaction_type comuni
-- ============================================================================
...
BEGIN
  IF NEW.referred_by_id IS NOT NULL THEN
    ...
    -- Prova con 'earned' invece di 'referral'
...
BEGIN
  ...
  INSERT INTO points_transactions (user_id, points, transaction_type, description)
  VALUES (v_referrer_id, v_points_awarded, 'earned',
  ...
  IF v_referrer_of_referrer_id IS NOT NULL THEN
    ...
    INSERT INTO points_transactions (user_id, points, transaction_type, description)
    VALUES (v_referrer_of_referrer_id, v_points_awarded, 'earned',
      'Rete indiretta: ' || COALESCE(p_first_name || ' ' || p_last_name, p_email));
  END IF;
END;
$$ LANGUAGE plpgsql;

SELECT '✅ Trigger MLM fixato - Usato transaction_type = earned' as status;
