-- =====================================================
-- FIX COMPLETO: Setup finale sistema email + compensi
-- CON INVIO EMAIL AUTOMATICO
-- =====================================================

-- STEP 1: Aggiungi colonne mancanti a points_transactions
...
-- STEP 2: Ricrea trigger con tutti i fix + INVIO EMAIL
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    IF v_organization_id IS NULL THEN
      ...
      BEGIN
        ...
        -- Se le variabili non sono configurate, usa i valori hardcoded
        IF v_supabase_url IS NULL THEN
          v_supabase_url := 'https://uchrjlngfzfibcpdxtky.supabase.co';
        END IF;
        
        SELECT content INTO v_response
        FROM http((
          'POST',
          v_supabase_url || '/functions/v1/send-organization-email',
          ARRAY[
            http_header('Content-Type', 'application/json'),
            http_header('Authorization', 'Bearer ' || COALESCE(v_supabase_anon_key, ''))
          ],
          'application/json',
          json_build_object(
        ...
        RAISE NOTICE '📧 Email send request sent for organization: %', v_organization_id;
        
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '⚠️ Failed to send email, but organization created successfully: %', SQLERRM;
      END;
...
-- STEP 3: Abilita l'estensione http per chiamare Edge Functions (se non già abilitata)
CREATE EXTENSION IF NOT EXISTS http;

SELECT '✅ Setup completo con invio email automatico!' as status;
