-- ============================================================================
-- TEST MLM COMPLETO - 3 LIVELLI (SENZA CLEANUP)
-- ============================================================================
...
DECLARE
  ...
  v_code_c VARCHAR(20);
  v_code_d VARCHAR(20);
  v_admin_points_before INTEGER;
  v_admin_points_after INTEGER;
BEGIN
  
  -- ========================================================================
  -- SETUP: Trova Akirayouky
  -- ========================================================================
  
  SELECT id INTO v_admin_id FROM users WHERE referral_code = 'ADMIN001';
  
  IF v_admin_id IS NULL THEN
    RAISE EXCEPTION '❌ Akirayouky (ADMIN001) non trovato!';
  END IF;
  
  -- Punti Akirayouky prima del test
  SELECT COALESCE(points_total, 0) INTO v_admin_points_before
  FROM user_points WHERE user_id = v_admin_id;
  ...
  RAISE NOTICE '   Akirayouky ID: %', v_admin_id;
  RAISE NOTICE '   Akirayouky punti iniziali: %', v_admin_points_before;
  RAISE NOTICE '';
  RAISE NOTICE '🎬 SCENARIO TEST:';
  RAISE NOTICE '   Akirayouky invita User B     → Akirayouky +1';
  RAISE NOTICE '   User B invita User C         → Akirayouky +1, User B +1';
  RAISE NOTICE '   User C invita User D         → User B +1, User C +1 (Akirayouky NO)';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 ATTESO FINALE:';
  RAISE NOTICE '   Akirayouky: % punti', v_admin_points_before + 2;
  ...
  -- ========================================================================
  -- STEP 1: Crea User B (referral diretto di Akirayouky)
  -- ========================================================================
  
  RAISE NOTICE '1️⃣  CREAZIONE USER B (referral di Akirayouky)...';
  ...
  -- Verifica punti Akirayouky dopo User B
  SELECT COALESCE(points_total, 0) INTO v_admin_points_after
  FROM user_points WHERE user_id = v_admin_id;
  
  RAISE NOTICE '';
  RAISE NOTICE '   📊 Akirayouky punti: % → % (diff: +%)', 
    v_admin_points_before, 
    v_admin_points_after,
    v_admin_points_after - v_admin_points_before;
  ...
  RAISE NOTICE '      Akirayouky: % (atteso: %)', 
    (SELECT COALESCE(points_total, 0) FROM user_points WHERE user_id = v_admin_id),
    v_admin_points_before + 2;
  ...
  RAISE NOTICE '   Akirayouky:  % punti (atteso: %)', 
    (SELECT COALESCE(points_total, 0) FROM user_points WHERE user_id = v_admin_id),
    v_admin_points_before + 2;
  ...
  RAISE NOTICE '   Akirayouky: % record', 
    (SELECT COUNT(*) FROM referral_network WHERE user_id = v_admin_id);
  ...
  RAISE NOTICE '   Akirayouky: % transazioni', 
    (SELECT COUNT(*) FROM points_transactions WHERE user_id = v_admin_id AND transaction_type = 'referral_completed');
