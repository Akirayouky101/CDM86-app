-- ═══════════════════════════════════════════════════════════════════
-- FIX: abandon_collaborator_role - bypassa trigger user_points
-- Il trigger su UPDATE di referred_by_id causa FK violation su
-- user_points. Si usa session_replication_role=replica per bypassarlo
-- solo durante gli UPDATE della catena MLM e del collaboratore stesso.
-- ═══════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    IF v_collab.user_id IS NULL THEN
        ...
        -- Bypassiamo i trigger per l'INSERT in public.users
    ...
    -- ══════════════════════════════════════════════════════════════
    -- 3. Genera nuova password casuale (8 caratteri)
    --    Ritornata al frontend che la applica via Auth Admin API
    -- ══════════════════════════════════════════════════════════════
    ...
    --    Bypassa trigger user_points con session_replication_role
    ...
    -- ══════════════════════════════════════════════════════════════
    -- 7. Aggiorna public.users e auth.users: rimuove ruolo collab
    --    Bypassa trigger per evitare side effects
    -- ══════════════════════════════════════════════════════════════
    SET session_replication_role = replica;
...
EXCEPTION WHEN OTHERS THEN
    -- Assicurati di ripristinare il replication role in caso di errore
    SET session_replication_role = DEFAULT;
