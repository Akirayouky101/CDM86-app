-- ═══════════════════════════════════════════════════════════════════
-- FUNZIONE: Abbandona Ruolo Collaboratore
-- ═══════════════════════════════════════════════════════════════════
-- Permette a un collaboratore di tornare utente normale mantenendo
-- i primi 2 livelli MLM, mentre il terzo livello viene riassegnato
-- al referral dell'admin
-- ═══════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.abandon_collaborator_role(
    p_collaborator_id UUID
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
    v_collab              RECORD;
    v_user                RECORD;
    v_auth_user_id        UUID;
    v_new_password        TEXT;
    v_admin_referral_id   UUID;
    v_reassigned_count    INT := 0;
    v_referral_code       TEXT;
    v_new_user_id         UUID;
BEGIN
    -- ══════════════════════════════════════════════════════════════
    -- 1. Recupera dati collaboratore
    -- ══════════════════════════════════════════════════════════════
    SELECT * INTO v_collab
    FROM public.collaborators
    WHERE id = p_collaborator_id;

    IF NOT FOUND THEN
        RETURN json_build_object(
            'success', false,
            'error', 'Collaboratore non trovato'
        );
    END IF;

    v_auth_user_id := v_collab.auth_user_id;

    -- ══════════════════════════════════════════════════════════════
    -- 2. Verifica se esiste già un record in public.users
    -- ══════════════════════════════════════════════════════════════
    IF v_collab.user_id IS NULL THEN

        -- Genera referral code univoco
        LOOP
            v_referral_code := UPPER(SUBSTRING(MD5(RANDOM()::TEXT || CLOCK_TIMESTAMP()::TEXT) FROM 1 FOR 8));
            EXIT WHEN NOT EXISTS (SELECT 1 FROM public.users WHERE referral_code = v_referral_code);
        END LOOP;

        -- Crea il record in public.users
        INSERT INTO public.users (
            auth_user_id,
            email,
            first_name,
            last_name,
            referral_code,
            points,
            referred_by_id
        )
        VALUES (
            v_auth_user_id,
            v_collab.email,
            COALESCE(v_collab.first_name, 'Utente'),
            COALESCE(v_collab.last_name, ''),
            v_referral_code,
            0,
            v_collab.referred_by_id
        )
        RETURNING id INTO v_new_user_id;

        -- Aggiorna il collaborator con il nuovo user_id
        UPDATE public.collaborators
        SET user_id = v_new_user_id
        WHERE id = p_collaborator_id;

        -- Ricarica v_user
        SELECT * INTO v_user FROM public.users WHERE id = v_new_user_id;

    ELSE
        SELECT * INTO v_user
        FROM public.users
        WHERE id = v_collab.user_id;
    END IF;

    -- ══════════════════════════════════════════════════════════════
    -- 3. Genera nuova password casuale (8 caratteri hex maiuscoli)
    --    MD5 è sempre disponibile senza estensioni esterne
    -- ══════════════════════════════════════════════════════════════
    v_new_password := UPPER(SUBSTRING(MD5(RANDOM()::TEXT || NOW()::TEXT || v_auth_user_id::TEXT) FROM 1 FOR 8));

    -- Aggiorna la password in auth.users
    UPDATE auth.users
    SET
        encrypted_password = crypt(v_new_password, gen_salt('bf')),
        updated_at = NOW()
    WHERE id = v_auth_user_id;

    -- ══════════════════════════════════════════════════════════════
    -- 4. Trova il referral dell'admin
    --    Se l'admin non ha un referral, usiamo l'admin stesso
    -- ══════════════════════════════════════════════════════════════
    SELECT referred_by_id INTO v_admin_referral_id
    FROM public.users
    WHERE id = 'e8cde03d-2aa6-4ea6-a29f-43f290ae00ce';

    IF v_admin_referral_id IS NULL THEN
        v_admin_referral_id := 'e8cde03d-2aa6-4ea6-a29f-43f290ae00ce';
    END IF;

    -- ══════════════════════════════════════════════════════════════
    -- 5. Riassegna il TERZO LIVELLO MLM al referral dell'admin
    -- Struttura:
    --   L1 = utenti con referred_by_id = collaboratore   → MANTIENI
    --   L2 = utenti con referred_by_id IN (L1)           → MANTIENI
    --   L3 = utenti con referred_by_id IN (L2)           → RIASSEGNA
    -- ══════════════════════════════════════════════════════════════
    WITH level_1 AS (
        SELECT id
        FROM public.users
        WHERE referred_by_id = v_user.id
    ),
    level_2 AS (
        SELECT u.id
        FROM public.users u
        INNER JOIN level_1 l1 ON u.referred_by_id = l1.id
    ),
    level_3 AS (
        SELECT u.id
        FROM public.users u
        INNER JOIN level_2 l2 ON u.referred_by_id = l2.id
    )
    UPDATE public.users
    SET referred_by_id = v_admin_referral_id
    WHERE id IN (SELECT id FROM level_3);

    GET DIAGNOSTICS v_reassigned_count = ROW_COUNT;

    -- ══════════════════════════════════════════════════════════════
    -- 6. Elimina il record da collaborators
    -- ══════════════════════════════════════════════════════════════
    DELETE FROM public.collaborators
    WHERE id = p_collaborator_id;

    -- ══════════════════════════════════════════════════════════════
    -- 7. Aggiorna metadata auth per rimuovere ruolo collaboratore
    -- ══════════════════════════════════════════════════════════════
    UPDATE auth.users
    SET raw_user_meta_data =
        COALESCE(raw_user_meta_data, '{}'::jsonb) ||
        '{"account_type": "user"}'::jsonb
    WHERE id = v_auth_user_id;

    -- Rimuove i flag collaboratore anche in public.users
    UPDATE public.users
    SET
        is_collaborator = false,
        collaborator_status = NULL,
        account_type = 'user'
    WHERE id = v_user.id;

    -- ══════════════════════════════════════════════════════════════
    -- 8. Ritorna risultato
    -- ══════════════════════════════════════════════════════════════
    RETURN json_build_object(
        'success', true,
        'message', 'Ruolo collaboratore abbandonato con successo',
        'data', json_build_object(
            'email',               v_collab.email,
            'new_password',        v_new_password,
            'user_id',             v_user.id,
            'referral_code',       v_user.referral_code,
            'level_3_reassigned',  v_reassigned_count,
            'new_referral_parent', v_admin_referral_id
        )
    );

EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'abandon_collaborator_role ERROR: % (STATE: %)', SQLERRM, SQLSTATE;
    RETURN json_build_object(
        'success', false,
        'error', SQLERRM,
        'state', SQLSTATE
    );
END;
$$;

-- ══════════════════════════════════════════════════════════════════
-- COMMENTI
-- ══════════════════════════════════════════════════════════════════
COMMENT ON FUNCTION public.abandon_collaborator_role(UUID) IS
'Permette a un collaboratore di abbandonare il ruolo e tornare utente normale.
- Mantiene i livelli MLM 1 e 2
- Riassegna il livello 3 al referral dell''admin
- Genera nuova password casuale
- Elimina il record da collaborators
- Restituisce JSON: success, email, new_password, referral_code, level_3_reassigned';

-- ══════════════════════════════════════════════════════════════════
-- GRANT
-- ══════════════════════════════════════════════════════════════════
GRANT EXECUTE ON FUNCTION public.abandon_collaborator_role(UUID) TO authenticated;
