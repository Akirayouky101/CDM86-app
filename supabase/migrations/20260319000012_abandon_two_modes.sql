-- Migration _0012: abandon_collaborator_role with two modes
-- Mode 'definitivo': all L1/L2/L3 → admin
-- Mode 'reiscritto': L1/L2 → new user (by referral code), L3 → admin
...
BEGIN
    ...
    -- Collect L1 (direct referrals of the collaborator)
    ...
    -- Count L3 (referrals of L2) — for the result report
    ...
    -- Update referral_network: change referred_by_user_id for L1
    IF v_l1_ids IS NOT NULL AND array_length(v_l1_ids, 1) > 0 THEN
        IF p_mode = 'definitivo' THEN
            UPDATE public.referral_network
            SET referred_by_user_id = v_admin_pub_id
            WHERE user_id = ANY(v_l1_ids);
        ELSE
            UPDATE public.referral_network
            SET referred_by_user_id = v_new_target_pub_id
            WHERE user_id = ANY(v_l1_ids);
        END IF;
    END IF;

    -- Update referral_network: change referred_by_user_id for L2
    IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
        IF p_mode = 'definitivo' THEN
            UPDATE public.referral_network
            SET referred_by_user_id = v_admin_pub_id
            WHERE user_id = ANY(v_l2_ids);
        ELSE
            UPDATE public.referral_network
            SET referred_by_user_id = v_new_target_pub_id
            WHERE user_id = ANY(v_l2_ids);
    ...
    -- Update referral_network: change referred_by_user_id for L3 (always → admin)
    IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
        UPDATE public.referral_network
        SET referred_by_user_id = v_admin_pub_id
        WHERE referred_by_user_id = ANY(v_l2_ids);
    END IF;
...
-- Drop old single-param overload if it exists (clean up previous signature)
DO $$
BEGIN
    -- Attempt to drop old single-param version; ignore if not found
    DROP FUNCTION IF EXISTS public.abandon_collaborator_role(UUID);
EXCEPTION WHEN OTHERS THEN
    -- ignore
END;
$$;
