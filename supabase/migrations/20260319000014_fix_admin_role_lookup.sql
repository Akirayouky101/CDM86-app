-- Migration _0014: fix admin lookup — use role='admin' instead of is_admin=true
...
BEGIN
    ...
    -- Get admin public.users.id using role = 'admin'
    ...
    IF p_mode = 'definitivo' THEN
        IF v_l1_ids IS NOT NULL AND array_length(v_l1_ids, 1) > 0 THEN
            UPDATE public.users SET referred_by_id = v_admin_pub_id WHERE id = ANY(v_l1_ids);
        END IF;
        IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
            UPDATE public.users SET referred_by_id = v_admin_pub_id WHERE id = ANY(v_l2_ids);
        END IF;
        IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
    ...
    ELSIF p_mode = 'reiscritto' THEN
        IF v_l1_ids IS NOT NULL AND array_length(v_l1_ids, 1) > 0 THEN
            UPDATE public.users SET referred_by_id = v_new_target_pub_id WHERE id = ANY(v_l1_ids);
        END IF;
        IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
            UPDATE public.users SET referred_by_id = v_new_target_pub_id WHERE id = ANY(v_l2_ids);
        END IF;
        IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
            UPDATE public.users SET referred_by_id = v_admin_pub_id
            WHERE referred_by_id = ANY(v_l2_ids);
    ...
    -- Structure: user_id = referrer, referral_id = the referred user
    IF p_mode = 'definitivo' THEN
        IF v_l1_ids IS NOT NULL AND array_length(v_l1_ids, 1) > 0 THEN
            UPDATE public.referral_network SET user_id = v_admin_pub_id
    ...
    ELSIF p_mode = 'reiscritto' THEN
        IF v_l1_ids IS NOT NULL AND array_length(v_l1_ids, 1) > 0 THEN
            UPDATE public.referral_network SET user_id = v_new_target_pub_id
            WHERE referral_id = ANY(v_l1_ids);
        END IF;
        IF v_l2_ids IS NOT NULL AND array_length(v_l2_ids, 1) > 0 THEN
            UPDATE public.referral_network SET user_id = v_new_target_pub_id
